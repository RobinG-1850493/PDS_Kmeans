...
class SerialKmeans {
  public:
      ...
      */
     double costFunction(vector<vector<double>> upCentroids,const vector<vector<double>> &data, vector<double> clusters, int k){
          double total = 0;
      ...
      */
     vector<double> getClosestCentroid(const vector<vector<double>> &data, vector<vector<double>> centroids, int k){
          ...
          {    
               ...
               for(int i = 0; i < data_size; i++){  
                    ...
                    lowestDist = 10000000000000000;
                    for(int j = 0; j < cluster_size; j++){
                         if(data[i] != centroids[j]){
                              double currentDist = euclidDistance(data[i], centroids[j]);
                    ...
                    priv_clusters.emplace_back(clusterOfLowestDistance);
               }
     ...
     */
     vector<vector<double>> updateCentroids(const vector<vector<double>> &data, vector<double> &clusters, int k){
          double total = 0;
          int cluster_size = 0;
          int temp = 0;
          vector<double> punt;
     ...
     */
     double euclidDistance(vector<double> punt, vector<double> centroid)
     {
          ...
          for(int i = 0; i < punt.size(); i++){
               dist += (punt[i] - centroid[i])*(punt[i] - centroid[i]);            
          }  
	     return dist;
     }
};
