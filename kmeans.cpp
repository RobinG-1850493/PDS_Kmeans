...
class SerialKmeans {
  public:
     vector<vector<double>> data, centroids, newCentroids, endClusters;
     ...
     SerialKmeans(vector<vector<double>> &data, string output, string trace, int k, int repetitions, int seed){
           ...
           */
          if(trace != ""){
               traceStream.open(trace, ios_base::app);
          }
          srand(seed);
     };
     ...
     void kMeans(){
          ...
          for(int i = 0; i < repetitions; i++){
               ...
               endClusters.push_back(clusters);
          }
     ...
     void writeOutputCSV(){
          ...
          for(int i = 0; i < endClusters.size(); i++){
               writer.write(endClusters[i]);
          } 
     }
